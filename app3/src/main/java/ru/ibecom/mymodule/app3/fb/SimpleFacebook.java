package ru.ibecom.mymodule.app3.fb;
...
import ru.ibecom.mymodule.app3.fb.entities.Album;
import ru.ibecom.mymodule.app3.fb.entities.Checkins;
import ru.ibecom.mymodule.app3.fb.entities.Feed;
import ru.ibecom.mymodule.app3.fb.entities.Photo;
import ru.ibecom.mymodule.app3.fb.entities.Profile;
import ru.ibecom.mymodule.app3.fb.entities.Story;
import ru.ibecom.mymodule.app3.fb.entities.Video;
import ru.ibecom.mymodule.app3.fb.utils.Errors;
import ru.ibecom.mymodule.app3.fb.utils.Errors.ErrorMsg;
import ru.ibecom.mymodule.app3.fb.utils.Logger;
...
public class SimpleFacebook
{
	 ...
	 * this method: {@link #getProfile(Properties, fb.SimpleFacebook.OnProfileRequestListener)} <br>
	 * <br>
	 ...
	 * It is recommended in this case, to use {@link #getProfile(Properties, fb.SimpleFacebook.OnProfileRequestListener)} and
	 ...
	 * this method: {@link #getFriends(Properties, fb.SimpleFacebook.OnFriendsRequestListener)} <br>
	 * <br>
	 * 
	...
	public void postScore(int score, final OnPostScoreListener onPostScoreListener)
	{
		if (isLogin())
		{
			...
			if (mConfiguration.getPublishPermissions().contains(Permissi.PUBLISH_ACTION.getValue()))
			{
				// callback with 'thinking'
				 ...
				 */
				if (!getOpenSessionPermissions().contains(Permissi.PUBLISH_ACTION.getValue()))
				{
	...
	public void publish(final Feed feed, final OnPublishListener onPublishListener)
	{
		...
		if (isLogin())
		{
			...
			{
				String reason = Errors.getError(ErrorMsg.PERMISSIONS_PUBLISH, Permissi.PUBLISH_ACTION.getValue());
				logError(reason, null);
	...
	/**
	 * Publish photo to specific album. You can use {@link #getAlbums(fb.SimpleFacebook.OnAlbumsRequestListener)} to retrieve
	...
	public void publish(final Photo photo, final String albumId, final OnPublishListener onPublishListener)
	{
		...
		{
			// if we defined the publish permission
			if (mConfiguration.getPublishPermissions().contains(Permissi.PUBLISH_STREAM.getValue()))
			{
				// callback with 'thinking'
				if (onPublishListener != null)
				{
				 ...
				 * this permission.
				 */
				if (!getOpenSessionPermissions().contains(Permissi.PUBLISH_STREAM.getValue()))
				{
					mSessionStatusCallback.mOnReopenSessionListener = new OnReopenSessionListener()
					{
						@Override
			...
			else
			{
				...
				if (onPublishListener != null)
				{
					String reason = Errors.getError(ErrorMsg.PERMISSIONS_PUBLISH, Permissi.PUBLISH_STREAM.getValue());
					logError(reason, null);
